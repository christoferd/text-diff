...
class WP_Text_Diff_Renderer_Table extends Text_Diff_Renderer
{
	...
	{
		return '<div class="diff-addedline">' . $line . '</div>';
	}
	...
	{
		return '<div class="diff-deletedline">' . $line . '</div>';
	}
	...
	public function _added($lines, $encode = true)
	{
		...
		foreach ($lines as $line) {
			...
			if ($this->_show_split_view) {
				$r .= '<div class="diff_row">' . $this->emptyLine() . $this->emptyLine() . $this->addedLine($line) . '</div>' . PHP_EOL;
			} else {
				$r .= '<div class="diff_row">' . $this->addedLine($line) . '</div>' . PHP_EOL;
			}
		}
		return $r;
	}
	...
	public function _deleted($lines, $encode = true)
	{
		...
		foreach ($lines as $line) {
			...
			if ($this->_show_split_view) {
				$r .= '<div class="diff_row">' . $this->deletedLine($line) . $this->emptyLine() . $this->emptyLine() . '</div>' . PHP_EOL;
			} else {
				$r .= '<div class="diff_row">' . $this->deletedLine($line) . '</div>' . PHP_EOL;
			}
		}
		return $r;
	}
	...
	public function _context($lines, $encode = true)
	{
		...
		foreach ($lines as $line) {
			...
			if ($this->_show_split_view) {
				$r .= '<div class="diff_row">' . $this->contextLine($line) . $this->emptyLine() . $this->contextLine($line)  . '</div>' . PHP_EOL;
			} else {
				$r .= '<div class="diff_row">' . $this->contextLine($line) . '</div>' . PHP_EOL;
			}
		}
		return $r;
	}
	...
	public function _changed($orig, $final)
	{
		...
		foreach (array_keys($orig_rows) as $row) {
			...
			if ($orig_rows[$row] < 0) { // Orig is blank. This is really an added row.
				...
				if ($this->_show_split_view) {
					$r .= '<div class="diff_row">' . $this->deletedLine($orig_line)
						. $this->emptyLine() . $this->addedLine($final_line) . '</div>' . PHP_EOL;
				} else {
					$r .= '<div class="diff_row">' . $this->deletedLine($orig_line)
						. '</div><div class="diff_row">' . $this->addedLine($final_line) . '</div>' . PHP_EOL;
				}
			}
		}
